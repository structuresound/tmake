...
describe('core.actions.packages', () => {
  describe('create', () => {
    ...
    it('should call Meteor.call to save the package', () => {
      ...
      expect(methodArgs.slice(0, 4)).to.deep.equal([
        'packages.create', 'id', 't', 'c'
      ]);
      expect(methodArgs[4]).to.be.a('function');
    });

    it('should redirect user to the package', () => {
      const id = 'dsds';
      ...
      expect(FlowRouter.go.args[0][0]).to.be.equal(`/package/${id}`);
    });
