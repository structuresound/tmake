_ = require('underscore')

fs = require('vinyl-fs')

module.exports = (transform) ->
  context =
    map: require('map-stream');
    src: (glob, opt) ->
      options = opt or {}
      options.cwd = transform.srcDir
      console.log 'src:', glob, options
      fs.src glob, options
    dest: fs.dest
  _.extend(context, transform.config);
  pipeline: ->
    if transform.pipeline
      transform.pipeline.apply(context)
    else
      console.log "transform", transform, "has no pipeline"
    # options = {}
    # walker = walk.walk(config.srcDir, config.walk)
    # walker.on 'file', (root, fileStats, next) ->
    #   filename = fileStats.name
    #   console.log 'walk', root + '/' + filename
    #   next()
    # walker.on 'errors', (root, nodeStatsArray, next) -> next()
    # walker.on 'end', -> console.log 'all done'
  # process_files: ->
  #   options = {}
  #   walker = walk.walk(src_dir, options)
  #   walker.on 'file', (root, fileStats, next) ->
  #     filename = fileStats.name
  #     if filename.endsWith('.c') or filename.endsWith('.h')
  #       fs.readFile filename, (err, data) ->
  #         replace_includes data
  #         next()
  #     else
  #       next()
  #   walker.on 'errors', (root, nodeStatsArray, next) -> next()
  #   walker.on 'end', -> console.log 'all done'
