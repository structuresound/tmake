...
declare module 'minimist' {
	...
	namespace minimist {
		export interface Opts {
			...
			// when true, populate args._ with everything after the first non-option
			stopEarly?: boolean;
			// a function which is invoked with a command line parameter not defined in the opts configuration object.
			// If the function returns false, the unknown option is not added to args
			unknown?: (arg: string) => boolean;
			// when true, populate args._ with everything before the -- and args['--'] with everything after the --
			'--'?: boolean;
		}
