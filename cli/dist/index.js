...
function version() {
    var packageInfo = tmake_file_1.parseFileSync(path.join(tmake_core_1.args.npmDir, 'package.json'));
    ...
    switch (command) {
        ...
        case 'report':
            return Db.getReports().then(function (reports) {
                reports.array.forEach(function (report) {
...
exports.tmake = tmake;
function initRepo() {
    if (!tmake_file_1.findConfigAsync(tmake_core_1.args.runDir)) {
        return createPackage().then(function (config) {
            return tmake_file_1.writeFileAsync(tmake_core_1.args.runDir + "/tmake.yaml", yaml.dump(config));
        });
    }
...
function run() {
    ...
    switch (tmake_core_1.args._[0]) {
        ...
        case 'manual':
            ...
            return;
        default:
            return tmake_file_1.readConfigAsync(tmake_core_1.args.configDir || tmake_core_1.args.runDir)
                .then(function (res) {
                if (res) {
                    var projectFile = res;
                    ...
                    switch (tmake_core_1.args._[0]) {
                        ...
                        case 'reset':
                        case 'nuke':
                            tmake_file_1.nuke(path.join(tmake_core_1.args.runDir, 'bin'));
                            tmake_file_1.nuke(path.join(tmake_core_1.args.runDir, 'build'));
                            tmake_file_1.nuke(path.join(tmake_core_1.args.runDir, tmake_core_1.args.cachePath));
                ...
                .catch(function (e) {
                try {
                    if (typed_json_transform_1.check(e, TMakeError)) {
                        e.postMortem();
                    }
                    else {
