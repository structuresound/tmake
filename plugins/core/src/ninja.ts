...
function build(configuration: TMake.Configuration.Post) {
  return Tools.fetch(configuration.tools).then((toolpaths: any) => {
  ...
  configure(): PromiseLike<any> {
    ...
    return this.sources()
      ...
      .then((libraries) => {
        ...
        // const relativeToBuild = path.relative(this.configuration.post.project.build, this.configuration.post.d.build) || '.';
        const relativeToSource = path.relative(this.configuration.post.d.build, this.configuration.post.d.source) || '.';
        ...
        const includeString = `-I ${this.configuration.post.d.root}/trie_modules/include` + _.map(this.options.includeDirs, (dir) => {
          return `-I${dir}`;
        }).join(' ');
        ...
        const name = this.configuration.project.post.name;
