...
export namespace Fetch {
  ...
  function upsertCache(project: TMake.Project) {
    return Runtime.Db.projectNamed(project.post.name).then((res: any) => {
      if (res) {
  ...
  function getSource(project: TMake.Project) {
    ...
    mkdir('-p', project.post.d.root);
    info.fetch.url(project);
    return download(url)
      .then((filePath) => { return unarchiveSource(filePath, project.post.d.clone); });
  }
  ...
  function linkSource(project: TMake.Project) {
    ...
    return new Bluebird<void>((resolve, reject) => {
      try {
        fs.symlink(url, project.post.d.root, 'dir', (err) => {
          if (err) {
            reject(err);
          }
          resolve();
  ...
  function destroy(project: TMake.Project) {
    try {
      ...
      file.nuke(project.post.d.clone);
    } catch (e) { }
  ...
  function maybeFetch(project: TMake.Project) {
    if (project.post.link) {
      return linkSource(project).then(() => Bluebird.resolve(true));
    }
    if (project.post.archive || project.post.git) {
      const exists = fs.existsSync(project.post.d.clone);
