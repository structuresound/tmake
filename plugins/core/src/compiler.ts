...
function resolveFlags(environment: TMake.Configuration, options: TMake.Plugin.Compiler.Options) {
...
export class Compiler extends Shell {
  ...
  projectFilePath() {
    return join(this.configuration.post.d.project, this.projectFileName);
  }
  buildFilePath() {
    return join(this.configuration.post.d.build, this.buildFileName);
  }
  ...
  sources() {
    ...
    return glob(patterns, configuration.post.d.source, configuration.project.post.d.source);
  }
  libraries(): PromiseLike<any> {
    const { require } = this.configuration.project.post
    if (require) {
      const stack = _.map(require, (dep: Project) => {
        ...
        return _.map(dep.cache.libs.value(), (lib) => {
          console.log('+', path.join(dep.post.d.home, lib));
          return path.join(dep.post.d.home, lib);
        })
      })
  ...
  public configure(): PromiseLike<any> {
    ...
    mkdir('-p', this.configuration.post.d.build);
