...
export function configure(configuration: Configuration, isTest?: boolean): PromiseLike<any> {
  const phase = new Phase(configuration.post.configure);
        ...
        switch (i.cmd) {
          ...
          case 'replace':
            return iterateOLHM(i.arg, (replEntry: ReplEntry) => {
              ...
              return file.glob(pattern, undefined, configuration.project.post.d.source)
                .then((files: string[]) => {
          ...
          case 'create':
            return iterateOLHM(
              i.arg, (entry: any) => {
                const filePath = join(configuration.project.post.d.source, entry.path);
  ...
  log.verbose(`configuration is current, use --force=${configuration.project.post.name} if you suspect the cache is stale`);
...
export function build(configuration: Configuration, isTest?: boolean): PromiseLike<any> {
  if (configuration.project.force() || configuration.cache.build.dirty()) {
    const phase = new Phase(configuration.post.build);
    return Bluebird.each(
      phase.commands,
      (i: TMake.CmdObj): PromiseLike<any> => {
        log.verbose(`    ${i.cmd}`);
