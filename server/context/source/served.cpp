...
void insertTest() {
  ...
  // We choose to move in our document here, which transfers ownership to
  // insert_one()
...
void queryTest() {
  ...
  // Query with the greater-than operator ($gt).
  {
    ...
    bsoncxx::builder::stream::document filter_builder;
    filter_builder << "grades.score" << open_document << "$gt" << 30
                   << close_document;
  ...
  // Query with the less-than operator ($lt).
  {
    ...
    bsoncxx::builder::stream::document filter_builder;
    filter_builder << "grades.score" << open_document << "$lt" << 10
                   << close_document;
  ...
  // Query with a logical disjunction (OR) of query conditions.
  {
    ...
    filter_builder << "$or" << open_array << open_document << "cuisine"
                   << "Italian" << close_document << open_document
                   << "address.zipcode"
